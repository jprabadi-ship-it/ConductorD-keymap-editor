        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowChangelog(false)}>
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 8, padding: 24, maxWidth: 520, maxHeight: '70vh', overflowY: 'auto', color: 'var(--text-primary)', fontSize: 13, lineHeight: 1.6 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { v: '0.38.55.0', at: '2026-09-15 JST', changes: ['シリアルポート選択ダイアログの「推奨」判定を修正。USB識別情報を持たないポート(Bluetooth-Incoming-Port、debug-console、BTイヤホン等)同士が同一デバイスの兄弟ポートとして誤ってグループ化され、まとめて推奨扱いになる一方、本物のConductorが無印になっていました。ConductorのUSB識別子(ZMK標準のVID 0x1D50/PID 0x615E)で直接判定するようにし、他のUSBシリアル機器が同時に繋がっていても正しく「推奨: Conductor」が付きます'] },
                { v: '0.38.54.0', at: '2026-09-15 JST', changes: ['macOS 27へのOSアップデートに合わせて内部コンポーネントを一括更新。アプリ基盤のElectronを43から44へ(新macOS SDK対応を含む)、BLEブリッジのnoble、React、Viteなども最新安定版に更新しました。機能変更はありません', 'ネイティブBLEモジュールが新Electronで正常に読み込めることを確認済みです'] },
                { v: '0.38.53.0', at: '2026-08-30 JST', changes: ['ミニマップとStudioでBLE接続を共有するように。ミニマップで接続してからStudioを開くと未接続表示になっていましたが、ネイティブBLEの接続はmainプロセスに1本しか存在しないため、後から開いた側が既存の接続を引き継ぐようにしました(逆方向も同様)。接続し直しは発生しません', 'ウィンドウごとにRPCリクエストIDの範囲を分離。1本のBLE接続を共有すると応答が両ウィンドウに配信されるため、従来は両方が1から採番していてIDが衝突し、相手の応答を自分のものと取り違える可能性がありました(USBは排他のため従来通りハンドオフ方式です)'] },
                { v: '0.38.52.0', at: '2026-08-30 JST', changes: ['USB接続が予期せず切れた場合、デバイスが復帰した時点で自動的に再接続するように。ファームウェアの書き込み・リセット・ケーブルの抜き差しでは毎回手動で接続し直す必要がありましたが、一度許可したポートは再度の操作なしで開けるため、Web Serialの再挿入イベントを拾って自動で開き直します(最大5回リトライ)。「Disconnect」ボタンで明示的に切断した場合は自動再接続しません。Studio本体・ミニマップの両方で有効', '複数のポートを許可済みの場合、再接続時は直前に使っていたポートを優先するように(従来は先頭のポートを開いていたため、デバッグ版ファームウェアではログ用ポートを掴む可能性がありました)'] },
                { v: '0.38.51.0', at: '2026-08-29 JST', changes: ['アプリ版のシリアルポート選択ダイアログで、どれを選べばよいか分かるようにラベルを追加。デバッグ版ファームウェアが見せる2本のポートは「推奨: Studio RPC」「ログ出力用と思われます」と表示し、推奨側を先頭かつ既定ボタンに配置します。macOSが常に出す幻の`Bluetooth-Incoming-Port`にも「USBデバイスではありません」と付記。判断材料がない場合(別々のUSB機器が複数ある等)は推測しません', 'Web版はブラウザ自身のポート選択画面のため表示を変更できないので、代わりに接続直後に応答確認を行い、ポートは開けたのに応答がない場合はその場で「ポート違いの可能性」を通知するようにしました(従来はWrite時まで気付けませんでした)'] },
                { v: '0.38.50.0', at: '2026-08-29 JST', changes: ['デバイスから応答がない(タイムアウト)ときに「デバイスがロックされています」と誤表示していた問題を修正。ロック状態の問い合わせが失敗した場合も「0=ロック中」として扱っていたため、実際はケーブル不良・シリアルポートの選び間違い・ファームウェアの不応答であっても、解決に繋がらないstudio_unlockコンボの操作を促していました。応答の有無を区別し、無応答時は原因の切り分け方(接続し直す、別のポートを選ぶ)を案内するメッセージに変更。Write・Timing・Trackball・コンボの全9箇所に反映'] },
                ...
                { v: '0.38.43.0', at: '2026-08-21 JST', changes: ['メニューバーアイコンを刷新: 背景の角丸プレートを廃してグリフを22ptいっぱいまで拡大し、macOSのテンプレート画像方式に変更。壁紙に応じてメニューバーが明るい時は黒、暗い時は白へOSが自動で反転させます(`scripts/gen_tray_icon.mjs`で再生成可能)', 'メニューバーアイコンのクリックが効かない問題に対処: 左クリックでもメニューが開くように変更(従来は右クリックのみで、左クリックでは無反応だった)。加えてシリアル/Bluetoothのデバイス選択ダイアログを同期(showMessageBoxSync)から非同期に変更し、可視ウィンドウにのみシート表示するように。同期ダイアログはメインプロセス全体を停止させるため、非表示ウィンドウにシートが付くと「画面には何も出ないのにアプリ全体が無反応」になり得た'] },
                ...
                { v: '0.38.30.0', at: '2026-07-24 JST', changes: ['トレイアイコンのレイアウトを「L残量 / アイコン / R残量」に変更(アイコンを中央、L/Rを左右に分離、以前は右側に2段でR/L)。フォントも太字から通常ウェイトに変更'] },
                { v: '0.38.29.0', at: '2026-07-24 JST', changes: ['ミニマップ(トレイのポップアップ)から直接接続した場合もトレイアイコンにR/Lバッテリーが表示されるように修正。これまではメインのStudioウィンドウでの接続時だけIPCでバッテリー情報を送っており、ミニマップ単体接続だと反映されなかった。切断時もトレイを「R--/L--」に戻すよう通知(Studio側で接続中の間は誤って切断表示にしないようガード付き)'] },
                { v: '0.38.28.0', at: '2026-07-24 JST', changes: ['トレイアイコンがしばらく使用後にメニューバーから消える不具合に対応。原因と推測されるのは、接続中(52px幅)⇄未接続時(元の22px幅アイコン)でアイコン幅が交互に変わることによるmacOSメニューバー配置の不安定化。常に52px幅の合成アイコンを使う(未接続時は「R--/L--」表示)よう変更し、再描画も最短15秒間隔にスロットル。プロセス自体はクラッシュしていなかった(ps auxで生存確認)ため、確定的な原因究明ではなく予防的対応'] },
